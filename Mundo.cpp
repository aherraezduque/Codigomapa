...
void Mundo::agregarListas()
{	
	//una vez que ya se ha cargado matriz tile
	for (int i = 0; i < MAX_TILES; i++)
	{
		if ((apuntando.celdas[i])->getTipo() == 3) {		//agregar a lista bonus
			carnet.agregar((apuntando.celdas[i]));
		}
		if ((apuntando.celdas[i])->getTipo() == 4) {		//agregar a lista obstaculo fijo
			fuegos.agregar((apuntando.celdas[i]));
		}
		if ((apuntando.celdas[i])->getTipo() == 5) {		//agregar a lista obstaculo movil
			llamas.agregar((apuntando.celdas[i]));
		}
	}
}
